...
#if defined(NDEBUG)
# error "Xunbaocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xba630f19ce49ffec6dc932f61704100cafd77c1c56d7cb0bfba90956b1f18745");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Xunbaocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Xunbaocoin Signed Message:\n";
...
{
    // Xunbaocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Xunbaocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Xunbaocoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Xunbaocoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Xunbaocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Xunbaocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x44786991600a0354b86fb71e02a23f552b3715442ad7ac447b869b7e5b024f17");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fd9e18f33b410606ffcb70bc6feef2db04ef4eab2ffa42b8a79af1b11a2d87509ffea5c8a8a664390fca3b75b1a81035cf894747010bd393e27b94799a370186") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519501196;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087072474;

        if (fTestNet)
        {
            block.nTime    = 1519501176;
            block.nNonce   = 385894299;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7f31525af8bb14be1bdd915d302b8ed2b0886e98cfee93e4ba2a77669dc8f78a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc1, 0xb1, 0xd1 }; // Xunbaocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XunbaocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XunbaocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static XunbaocoinMiner(CWallet *pwallet)
{
    printf("XunbaocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xunbaocoin-miner");
    ...
    try { loop {
        ...
        printf("Running XunbaocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("XunbaocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&XunbaocoinMiner, pwallet));
}
